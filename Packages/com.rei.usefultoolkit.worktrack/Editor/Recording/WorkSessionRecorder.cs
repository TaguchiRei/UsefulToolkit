...
namespace UsefulToolkit.WorkTrack
{
    ...
    /// CurrentSession.jsonへの書き込みはEditorの起動時と終了時のみに限定しており、
    /// 途中経過を定期的に書き戻すことはしない(意図的な設計)。
    /// そのため、クラッシュ等で終了処理が走らなかった場合、正確な終了時刻はわからない。
    /// その場合は開始時刻をそのまま終了時刻として扱い、作業時間0として確定する。
    ///
