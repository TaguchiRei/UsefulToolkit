#ifndef SANDBOX_TOON_REALISTIC_OUTLINE_PASS_INCLUDED
#define SANDBOX_TOON_REALISTIC_OUTLINE_PASS_INCLUDED

#include "ToonRealisticInput.hlsl"

// アウトライン幅（ピクセル）の基準とする画面の高さ
#define REALISTIC_OUTLINE_REFERENCE_HEIGHT 1080.0
...
OutlineVaryings RealisticOutlineVertex(OutlineAttributes input)
{
    ...
    float widthPx = _OutlineWidth * (_ScreenParams.y / REALISTIC_OUTLINE_REFERENCE_HEIGHT) * fade;
...
// アウトライン色。_OutlineBaseColorBlend で表面色を暗くした色へ寄せる
half4 RealisticOutlineFragment(OutlineVaryings input) : SV_Target
{
    UNITY_SETUP_INSTANCE_ID(input);
    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);

    half3 base = SampleRealisticBase(input.uv).rgb;
    half3 tinted = base * 0.25h;
